-- Step 1: Fix the foreign key constraint issue by cleaning up orphaned references
-- and standardizing the clearing logic

-- First, let's see what we have in behavior_history that might be causing issues
-- We'll then update the admin_clear_all_queues function to handle this properly

-- Update admin_clear_all_queues to handle the foreign key constraint properly
...
-- Create a new function for clearing individual items that follows the same pattern
