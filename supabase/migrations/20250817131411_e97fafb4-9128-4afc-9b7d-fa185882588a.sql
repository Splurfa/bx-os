...
-- Remove constraint and add super admin support

-- Step 1: Drop the existing role check constraint
ALTER TABLE profiles DROP CONSTRAINT IF EXISTS profiles_role_check;

-- Step 2: Create role_type enum (check if exists first)
DO $$
BEGIN
...
-- Step 3: Create/Update Zach's super admin profile (using TEXT for now)
DO $$
BEGIN
...
-- Step 4: Update admin_update_user_role function to support super_admin
...
-- Step 5: Update existing RLS policies to include super_admin
...
-- Step 6: Add new role constraint that includes super_admin
