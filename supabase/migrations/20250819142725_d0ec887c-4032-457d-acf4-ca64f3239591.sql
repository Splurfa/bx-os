...
-- Ensure behavior_requests can be created and updated for kiosk operations
-- Allow anonymous inserts for behavior requests (needed for kiosk operations)
CREATE POLICY IF NOT EXISTS "kiosk_behavior_request_insert" ON behavior_requests
FOR INSERT 
TO anon
WITH CHECK (true);

-- Allow anonymous updates for behavior requests (needed for status changes)
CREATE POLICY IF NOT EXISTS "kiosk_behavior_request_update" ON behavior_requests  
FOR UPDATE
TO anon
USING (true);

-- Allow anonymous reads for behavior requests (needed for kiosk queue management)
CREATE POLICY IF NOT EXISTS "kiosk_behavior_request_select" ON behavior_requests
FOR SELECT
TO anon  
USING (true);

-- Allow anonymous operations on reflections for kiosk submissions
CREATE POLICY IF NOT EXISTS "kiosk_reflection_operations" ON reflections
FOR ALL
TO anon
USING (true)
WITH CHECK (true);
...
CREATE POLICY IF NOT EXISTS "kiosk_status_access" ON kiosks
FOR SELECT
TO anon
USING (true);