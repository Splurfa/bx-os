-- Fix CSV Import Migration: Handle name column constraints properly
-- Drop the previous function and recreate with correct column handling

DROP FUNCTION IF EXISTS import_hillel_csv_data();

-- Create corrected import function that doesn't try to insert into generated name columns
...
BEGIN
    ...
    -- Create guardians for each family (excluding name column to let it auto-generate)
    ...
    -- Create students for each family (excluding name column to let it auto-generate)
    ...
    IF family_id IS NOT NULL THEN
        INSERT INTO students (family_id, first_name, last_name, grade, class_name, date_of_birth)
        VALUES 
            (family_id, 'Asher', 'Abramson', '7th', '7th', '2012-11-30');
