...
-- Update the archive function with proper search_path
...
-- Replace the old archive trigger with the new one
DROP TRIGGER IF EXISTS archive_on_approval ON public.behavior_requests;
CREATE TRIGGER archive_on_approval_with_history
  AFTER DELETE ON public.behavior_requests
  FOR EACH ROW
  EXECUTE FUNCTION public.archive_completed_intervention_with_history();