-- Fix admin_clear_all_queues to include 'review' status items
...
BEGIN
  -- Single transaction with CTE capturing exact deletion set - NOW INCLUDING 'review' status
  ...
  INSERT INTO user_sessions (
    ...
    jsonb_build_object(
      ...
      'timestamp', now(),
      'status', 'success',
      'version', '6.0_review_status_support',
      ...
      'note', 'Fixed to include review status items - CTE transaction with FK constraint guard'
    ),
    'Admin Dashboard',
    'completed'
  );
...
-- Fix clear_teacher_queue to include 'review' status items
...
DECLARE
  ...
  remaining_kiosk_refs INTEGER := 0;
BEGIN
  -- Single transaction with CTE capturing exact deletion set for teacher - NOW INCLUDING 'review' status
  WITH requests_to_delete AS (
    SELECT id FROM behavior_requests 
    WHERE teacher_id = p_teacher_id AND status IN ('waiting', 'active', 'review')
  ),
  kiosks_cleared AS (
    UPDATE kiosks 
    SET current_behavior_request_id = NULL,
  ...
  IF remaining_kiosk_refs > 0 THEN
    RAISE EXCEPTION 'FK constraint guard failed: % kiosks still reference deleted request IDs for teacher %', remaining_kiosk_refs, p_teacher_id;
  END IF;
