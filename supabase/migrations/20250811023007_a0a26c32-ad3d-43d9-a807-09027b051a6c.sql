-- Run previously approved function updates
...
-- Clear teacher queue function (idempotent)
CREATE OR REPLACE FUNCTION public.clear_teacher_queue(p_teacher_id uuid DEFAULT NULL)
 RETURNS integer
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO ''
...
BEGIN
  ...
  -- Security check
  IF current_user_role NOT IN ('teacher', 'admin') THEN
    RAISE EXCEPTION 'Access denied: Only teachers and admins can clear the queue';
  END IF;
  
  -- Determine target teacher
  IF current_user_role = 'admin' THEN
    -- Admins can clear any teacher's queue or all queues
    target_teacher_id := COALESCE(p_teacher_id, auth.uid());
  ELSE
    -- Teachers can only clear their own queue
    target_teacher_id := auth.uid();
  END IF;
  
  -- Use advisory lock to prevent concurrent operations
  IF NOT pg_try_advisory_lock(12347) THEN
    RAISE EXCEPTION 'Queue clear operation already in progress';
  END IF;
  
  BEGIN
    -- Delete all behavior requests for the target teacher
    -- This will trigger the archive function for each deleted row
    DELETE FROM public.behavior_requests
    WHERE teacher_id = target_teacher_id
      AND status IN ('waiting', 'review', 'completed');
    
    GET DIAGNOSTICS deleted_count = ROW_COUNT;
    
    -- Clean up any orphaned reflections (shouldn't happen with proper cascade)
    DELETE FROM public.reflections
    WHERE behavior_request_id NOT IN (
      SELECT id FROM public.behavior_requests
    );
    
    -- Trigger reassignment for remaining students
    PERFORM public.reassign_waiting_students();
    
    RAISE NOTICE 'Cleared % behavior requests for teacher %', deleted_count, target_teacher_id;
    
  EXCEPTION
    WHEN OTHERS THEN
      RAISE EXCEPTION 'Error clearing queue for teacher %: %', target_teacher_id, SQLERRM;
  END;
  
  -- Always release the advisory lock
  PERFORM pg_advisory_unlock(12347);
  
  RETURN deleted_count;
END;
$function$;

-- One-time data repair: clean any invalid kiosk assignments and reassign waiting students
SELECT public.reassign_waiting_students();