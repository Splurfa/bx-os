...
DROP FUNCTION public.admin_clear_all_queues();
DROP FUNCTION public.clear_teacher_queue(uuid);

-- Create new admin_clear_all_queues with CTE transaction and FK guard
...
-- Create new clear_teacher_queue with CTE transaction and FK guard  
...
DECLARE
  ...
  remaining_kiosk_refs INTEGER := 0;
BEGIN
  -- Single transaction with CTE capturing exact deletion set for teacher
  WITH requests_to_delete AS (
    SELECT id FROM behavior_requests 
