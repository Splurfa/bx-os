...
-- Recreate the student_profile_complete view
...
SELECT 
    ...
    (SELECT COUNT(*) FROM behavior_requests br WHERE br.student_id = s.id) as current_incidents,
    (SELECT COUNT(*) FROM reflections r 
     JOIN behavior_requests br ON r.behavior_request_id = br.id 
     WHERE br.student_id = s.id) as current_reflections
FROM students s
