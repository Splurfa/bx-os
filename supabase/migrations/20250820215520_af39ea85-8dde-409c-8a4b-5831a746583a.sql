-- Fix Bug #1: Admin Clear All Queues Function - Proper Deletion Order
-- This addresses foreign key constraint violations by ensuring proper deletion sequence
...
BEGIN
  ...
  -- Step 4: Update history records with reflection IDs if they existed (optional, for completeness)
  -- This step is safe because reflections are already deleted
  UPDATE behavior_history 
  SET reflection_id = NULL 
  WHERE behavior_request_id IN (
    SELECT id FROM behavior_requests WHERE status IN ('waiting', 'active')
  );
  
  -- Step 5: Finally delete the behavior requests (now safe)
