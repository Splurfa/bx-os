...
-- Create temporary table to track created families during processing
CREATE TABLE public.temp_family_tracking (
...
BEGIN
  ...
  DELETE FROM temp_family_tracking;
  ...
  LOOP
    ...
    -- Check if family already exists in tracking table
    SELECT family_id INTO family_id FROM temp_family_tracking WHERE temp_family_tracking.family_key = process_csv_to_families_and_students.family_key;
    ...
    IF family_id IS NULL THEN
      ...
      -- Track this family
      INSERT INTO temp_family_tracking (family_key, family_id) VALUES (process_csv_to_families_and_students.family_key, family_id);
    ...
    INSERT INTO students (
      ...
      family_id,
      TRIM(csv_row.student_first_name),
      TRIM(csv_row.student_last_name),
      csv_row.class_raw,
      csv_row.class_raw,
      student_dob
    );
  ...
  DROP TABLE temp_family_tracking;
...
EXCEPTION
  WHEN OTHERS THEN
    -- Clean up tracking table in case of error
    DROP TABLE IF EXISTS temp_family_tracking;
