...
interface AccountabilitySliderProps {
  value: number;
  onChange: (value: number) => void;
  label?: string;
}

const AccountabilitySlider = ({ value, onChange, label = "How much do you take responsibility?" }: AccountabilitySliderProps) => {
  ...
  const getAccountabilityState = () => {
    if (currentValue === 1) return 'none';
    if (currentValue === 2) return 'little';
    if (currentValue === 3) return 'some';
    if (currentValue === 4) return 'most';
    return 'full';
  };
  ...
  const levels = [
    ...
    { id: 'full', label: 'Full', icon: '✅', value: 5 }
  ];
      ...
      <div className="relative px-2 mb-4 h-8">
        ...
        <div 
          ...
          style={{ 
            ...
            borderColor: getAccountabilityColor()
          }}
        />
      </div>
        ...
        {levels.map((level) => (
          <div 
            ...
            className={`text-center transition-all duration-300 ${
              getAccountabilityState() === level.id ? 'opacity-100 scale-110' : 'opacity-60 scale-95'
            }`}
          >
...
export default AccountabilitySlider;